...
int server(){
    ...
    if ((sd.server_fd = socket(AF_INET, SOCK_STREAM, 0))
        == 0) {
    ...
    if (setsockopt(sd.server_fd, SOL_SOCKET,
                   SO_REUSEADDR | SO_REUSEPORT, &sd.opt,
                   sizeof(sd.opt))) {
        perror("setsockopt");
        exit(EXIT_FAILURE);
    }
    ...
    // Forcefully attaching socket to the port 8080
    if (bind(sd.server_fd, (struct sockaddr*)&sd.address,
             sizeof(sd.address))
        < 0) {
        perror("bind failed");
        exit(EXIT_FAILURE);
...
{
  std::cout << "before starting server";
  /// server
  server();
  printf("server started");
    sd.valread = read(sd.sockets[1], sd.buffer, 1024);
    printf("%s\n", sd.buffer);
    send(sd.sockets[0], sd.hello, strlen(sd.hello), 0);
